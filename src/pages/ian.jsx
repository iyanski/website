import React from "react"
import { Link, graphql, useStaticQuery } from "gatsby"
import LinkedInIcon from "../components/icons/LinkedInIcon"
import GithubIcon from "../components/icons/GithubIcon"

function FadeIn({ children, className = "", direction, delay }) {
  return <div className={className}>{children}</div>
}

const cardAccents = [
  "from-zinc-900 to-zinc-700",
  "from-emerald-700 to-zinc-800",
  "from-zinc-800 to-zinc-500",
]

export default function PersonalWebsiteLayout() {
  const { allMarkdownRemark } = useStaticQuery(graphql`
    query LatestPosts {
      allMarkdownRemark(sort: { frontmatter: { date: DESC } }, limit: 3) {
        nodes {
          id
          excerpt(pruneLength: 140)
          timeToRead
          fields {
            slug
          }
          frontmatter {
            title
            description
            date(formatString: "MMM D, YYYY")
          }
        }
      }
    }
  `)

  const posts = allMarkdownRemark.nodes

  const showcaseItems = [
    {
      type: "Company",
      title: "Giftees",
      description:
        "A gifting platform concept focused on helping businesses drive sales through digital gift card experiences and promotional campaigns.",
    },
    {
      type: "Company",
      title: "TapForCare",
      description:
        "A healthcare-focused platform concept centered on specialist consultations, patient support, and care coordination.",
    },
    {
      type: "Company",
      title: "StatsCentral",
      description:
        "The company enables sports organizations to operate more efficiently through a platform offering real-time data tracking, automated live stream overlays, and player profile management.",
    },
  ]

  const projects = [
    {
      title: "Giftees",
      category: "Commerce / Gifting Platform",
      summary: showcaseItems[0].description,
    },
    {
      title: "TapForCare",
      category: "Healthcare / Care Coordination",
      summary: showcaseItems[1].description,
    },
    {
      title: "StatsCentral",
      category: "Sports Data / SaaS Platform",
      summary: showcaseItems[2].description,
    },
  ]

  const values = [
    {
      step: "01",
      title: "Built for real-world use",
      text: "Projects shaped around actual business problems, workflows, and customer needs.",
    },
    {
      step: "02",
      title: "Systems-minded execution",
      text: "Work that combines product thinking, engineering discipline, and operational clarity.",
    },
    {
      step: "03",
      title: "From concept to delivery",
      text: "A portfolio spanning software products, startup ideas, automation, and platform design.",
    },
    {
      step: "04",
      title: "Clean and scalable",
      text: "Solutions designed with maintainability, usability, and long-term growth in mind.",
    },
  ]

  return (
    <div className="min-h-screen bg-white text-zinc-900 antialiased">
      <header className="sticky top-0 z-50 border-b border-zinc-200/80 bg-white/90 backdrop-blur">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4 lg:px-8">
          <a
            href="#home"
            className="text-sm font-semibold uppercase tracking-[0.2em] text-zinc-900"
          >
            ian tusil
          </a>

          <nav className="hidden items-center gap-8 md:flex">
            <a
              href="#writing"
              className="text-sm text-zinc-600 transition hover:text-zinc-900"
            >
              Writing
            </a>
            <a
              href="#work"
              className="text-sm text-zinc-600 transition hover:text-zinc-900"
            >
              Work
            </a>
            <a
              href="#approach"
              className="text-sm text-zinc-600 transition hover:text-zinc-900"
            >
              Approach
            </a>
            <a
              href="#about"
              className="text-sm text-zinc-600 transition hover:text-zinc-900"
            >
              About
            </a>
            <Link
              to="/blog"
              className="text-sm text-zinc-600 transition hover:text-zinc-900"
            >
              Blog
            </Link>
          </nav>

          <div className="flex items-center gap-4">
            <a
              href="https://www.linkedin.com/in/iyanski/"
              target="_blank"
              rel="noopener noreferrer"
              className="text-zinc-500 transition hover:text-zinc-900"
              aria-label="LinkedIn"
            >
              <LinkedInIcon />
            </a>
            <a
              href="https://github.com/iyanski"
              target="_blank"
              rel="noopener noreferrer"
              className="text-zinc-500 transition hover:text-zinc-900"
              aria-label="GitHub"
            >
              <GithubIcon />
            </a>
            <a
              href="#contact"
              className="rounded-full bg-zinc-900 px-4 py-2 text-sm font-medium text-white transition hover:bg-zinc-700"
            >
              Contact
            </a>
          </div>
        </div>
      </header>

      <main id="home">
        {/* Hero */}
        <section className="relative overflow-hidden">
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_left,_rgba(0,0,0,0.04),_transparent_30%),radial-gradient(circle_at_bottom_right,_rgba(0,0,0,0.05),_transparent_28%)]" />
          <div className="mx-auto grid max-w-7xl gap-12 px-6 py-24 lg:grid-cols-12 lg:px-8 lg:py-32">
            <FadeIn
              className="relative lg:col-span-7"
              direction="up"
              delay={100}
            >
              <div className="mb-6 inline-flex rounded-full border border-zinc-200 bg-zinc-50 px-3 py-1 text-xs font-medium text-zinc-600">
                It's me, Ian Tusil
              </div>
              <h1 className="max-w-4xl text-5xl font-semibold leading-tight tracking-tight text-zinc-950 md:text-6xl">
                I build digital products, platforms, and systems for modern
                businesses.
              </h1>
              <p className="mt-6 max-w-2xl text-lg leading-8 text-zinc-600">
                This site is a curated showcase of my work across software
                products, startup ventures, automation, integrations, and
                digital business ideas.
              </p>

              <div className="mt-10 flex flex-col gap-4 sm:flex-row">
                <a
                  href="#work"
                  className="rounded-full bg-zinc-900 px-6 py-3 text-sm font-medium text-white transition hover:bg-zinc-700"
                >
                  Explore my work
                </a>
                <a
                  href="#about"
                  className="rounded-full border border-zinc-300 px-6 py-3 text-sm font-medium text-zinc-900 transition hover:border-zinc-900"
                >
                  About me
                </a>
              </div>
            </FadeIn>

            <FadeIn
              className="relative lg:col-span-5"
              direction="up"
              delay={300}
            >
              <div className="rounded-[2rem] border border-zinc-200 bg-zinc-50 p-6 shadow-sm">
                <div className="rounded-[1.5rem] border border-zinc-200 bg-white p-6">
                  <div className="flex items-center justify-between border-b border-zinc-100 pb-4">
                    <div>
                      <p className="text-sm font-medium text-zinc-500">
                        At a glance
                      </p>
                      <p className="mt-1 text-lg font-semibold text-zinc-950">
                        Portfolio highlights
                      </p>
                    </div>
                    <div className="h-3 w-3 rounded-full bg-emerald-500" />
                  </div>

                  <div className="space-y-4 pt-5">
                    {showcaseItems.map(item => (
                      <div
                        key={item.title}
                        className="flex items-start gap-3 rounded-2xl border border-zinc-100 p-4"
                      >
                        <div className="mt-1 h-2.5 w-2.5 rounded-full bg-zinc-900" />
                        <div>
                          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-zinc-400">
                            {item.type}
                          </p>
                          <p className="mt-1 text-sm font-medium text-zinc-900">
                            {item.title}
                          </p>
                          <p className="mt-2 text-sm leading-6 text-zinc-600">
                            {item.description}
                          </p>
                        </div>
                      </div>
                    ))}
                  </div>
                </div>
              </div>
            </FadeIn>
          </div>
        </section>

        {/* Tag line strip */}
        <section className="border-y border-zinc-200 bg-zinc-50/60">
          <div className="mx-auto grid max-w-7xl gap-6 px-6 py-10 md:grid-cols-3 lg:px-8">
            {[
              "Software products and startup ventures",
              "Automation, integration, and systems thinking",
              "A focused portfolio with business-oriented execution",
            ].map((item, i) => (
              <FadeIn key={item} direction="up" delay={i * 100}>
                <div className="text-sm font-medium text-zinc-600">{item}</div>
              </FadeIn>
            ))}
          </div>
        </section>

        {/* Writing */}
        <section id="writing" className="mx-auto max-w-7xl px-6 py-24 lg:px-8">
          <FadeIn direction="up">
            <div className="flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
              <div className="max-w-2xl">
                <p className="text-sm font-semibold uppercase tracking-[0.2em] text-zinc-500">
                  Writing
                </p>
                <h2 className="mt-4 text-3xl font-semibold tracking-tight text-zinc-950 md:text-4xl">
                  Notes on systems, integration, and automation that holds up in
                  production.
                </h2>
              </div>
              <Link
                to="/blog"
                className="shrink-0 rounded-full border border-zinc-300 px-5 py-2.5 text-sm font-medium text-zinc-900 transition hover:border-zinc-900"
              >
                All articles →
              </Link>
            </div>
          </FadeIn>

          {posts.length === 0 ? (
            <p className="mt-14 text-zinc-500">No posts yet.</p>
          ) : (
            <div className="mt-14 grid gap-6 md:grid-cols-3">
              {posts.map((post, i) => (
                <FadeIn key={post.id} direction="up" delay={i * 120}>
                  <Link
                    to={`/blog${post.fields.slug}`}
                    className="group flex h-full flex-col overflow-hidden rounded-[1.75rem] border border-zinc-200 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-md"
                  >
                    <div
                      className={`relative h-40 bg-gradient-to-br ${
                        cardAccents[i % cardAccents.length]
                      }`}
                    >
                      <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_right,_rgba(255,255,255,0.18),_transparent_60%)]" />
                      <span className="absolute bottom-4 left-5 rounded-full bg-white/15 px-3 py-1 text-[0.65rem] font-semibold uppercase tracking-[0.18em] text-white backdrop-blur">
                        Article
                      </span>
                    </div>

                    <div className="flex flex-1 flex-col p-7">
                      <div className="flex items-center gap-2 text-xs font-medium text-zinc-400">
                        <time>{post.frontmatter.date}</time>
                        <span aria-hidden="true">·</span>
                        <span>{post.timeToRead} min read</span>
                      </div>
                      <h3 className="mt-3 text-lg font-semibold leading-snug text-zinc-950 transition group-hover:text-zinc-600">
                        {post.frontmatter.title}
                      </h3>
                      <p className="mt-3 line-clamp-4 text-sm leading-7 text-zinc-600">
                        {post.frontmatter.description || post.excerpt}
                      </p>
                      <span className="mt-6 inline-flex text-sm font-medium text-zinc-900 transition group-hover:text-zinc-500">
                        Read more →
                      </span>
                    </div>
                  </Link>
                </FadeIn>
              ))}
            </div>
          )}
        </section>

        {/* Work */}
        <section id="work" className="bg-zinc-950 text-white">
          <div className="mx-auto max-w-7xl px-6 py-24 lg:px-8">
            <FadeIn direction="up">
              <div className="max-w-2xl">
                <p className="text-sm font-semibold uppercase tracking-[0.2em] text-zinc-400">
                  Selected Projects
                </p>
                <h2 className="mt-4 text-3xl font-semibold tracking-tight md:text-4xl">
                  A portfolio of projects, companies, and product ideas shaped
                  by strategy, engineering, and execution.
                </h2>
              </div>
            </FadeIn>

            <div className="mt-14 grid gap-6 lg:grid-cols-3">
              {projects.map((project, index) => (
                <FadeIn
                  key={`${project.title}-${index}`}
                  direction="up"
                  delay={index * 150}
                >
                  <div className="flex h-full flex-col rounded-[1.75rem] border border-white/10 bg-white/5 p-7">
                    <div className="mb-8 flex h-44 items-center justify-center rounded-[1.25rem] border border-white/10 bg-gradient-to-br from-white/10 to-white/0">
                      <span className="text-5xl font-semibold tracking-tight text-white/25">
                        {project.title.slice(0, 1)}
                      </span>
                    </div>
                    <p className="text-xs font-semibold uppercase tracking-[0.18em] text-zinc-400">
                      {project.category}
                    </p>
                    <h3 className="mt-3 text-xl font-semibold">
                      {project.title}
                    </h3>
                    <p className="mt-4 text-sm leading-7 text-zinc-300">
                      {project.summary}
                    </p>
                  </div>
                </FadeIn>
              ))}
            </div>
          </div>
        </section>

        {/* Approach */}
        <section id="approach" className="mx-auto max-w-7xl px-6 py-24 lg:px-8">
          <div className="grid gap-12 lg:grid-cols-12">
            <FadeIn className="lg:col-span-4" direction="left">
              <p className="text-sm font-semibold uppercase tracking-[0.2em] text-zinc-500">
                Approach
              </p>
              <h2 className="mt-4 text-3xl font-semibold tracking-tight text-zinc-950 md:text-4xl">
                The principles behind the way I design, build, and present my
                work.
              </h2>
            </FadeIn>

            <div className="grid gap-6 lg:col-span-8 md:grid-cols-2">
              {values.map((item, i) => (
                <FadeIn key={item.step} direction="up" delay={i * 100}>
                  <div className="rounded-[1.75rem] border border-zinc-200 p-7">
                    <div className="text-sm font-semibold text-zinc-400">
                      {item.step}
                    </div>
                    <h3 className="mt-4 text-xl font-semibold text-zinc-950">
                      {item.title}
                    </h3>
                    <p className="mt-3 text-sm leading-7 text-zinc-600">
                      {item.text}
                    </p>
                  </div>
                </FadeIn>
              ))}
            </div>
          </div>
        </section>

        {/* About */}
        <section id="about" className="border-t border-zinc-200">
          <div className="mx-auto grid max-w-7xl gap-12 px-6 py-24 lg:grid-cols-12 lg:px-8">
            <FadeIn className="lg:col-span-4" direction="left" delay={0}>
              <p className="text-sm font-semibold uppercase tracking-[0.2em] text-zinc-500">
                About
              </p>
            </FadeIn>
            <FadeIn className="lg:col-span-8" direction="right" delay={150}>
              <h2 className="max-w-3xl text-3xl font-semibold tracking-tight text-zinc-950 md:text-4xl">
                I'm a builder focused on practical digital products, thoughtful
                systems, and execution that creates real value.
              </h2>
              <p className="mt-6 max-w-3xl text-base leading-8 text-zinc-600">
                My portfolio brings together startup concepts, software
                platforms, automation work, and integration-driven projects. The
                common thread is a strong focus on business utility, clean
                execution, and building things that are both useful and
                scalable.
              </p>
            </FadeIn>
          </div>
        </section>

        {/* Contact */}
        <section id="contact" className="mx-auto max-w-7xl px-6 pb-24 lg:px-8">
          <FadeIn direction="up">
            <div className="rounded-[2rem] bg-zinc-950 px-8 py-14 text-white md:px-12">
              <p className="text-sm font-semibold uppercase tracking-[0.2em] text-zinc-400">
                Contact
              </p>
              <div className="mt-4 flex flex-col gap-8 md:flex-row md:items-end md:justify-between">
                <div>
                  <h2 className="max-w-2xl text-3xl font-semibold tracking-tight md:text-4xl">
                    Interested in my work or want to explore a collaboration?
                  </h2>
                  <p className="mt-4 max-w-2xl text-base leading-7 text-zinc-300">
                    I'm open to conversations around product development,
                    systems design, automation, integrations, and digital
                    business opportunities.
                  </p>
                </div>
                <div className="flex flex-col gap-3 sm:flex-row">
                  <a
                    href="mailto:hello@iantusil.com"
                    className="rounded-full bg-white px-6 py-3 text-sm font-medium text-zinc-950 transition hover:bg-zinc-200"
                  >
                    Drop a Message
                  </a>
                  <a
                    href="https://www.linkedin.com/in/iyanski/"
                    className="rounded-full border border-white/20 px-6 py-3 text-sm font-medium text-white transition hover:border-white"
                  >
                    LinkedIn
                  </a>
                </div>
              </div>
            </div>
          </FadeIn>
        </section>
      </main>
    </div>
  )
}
